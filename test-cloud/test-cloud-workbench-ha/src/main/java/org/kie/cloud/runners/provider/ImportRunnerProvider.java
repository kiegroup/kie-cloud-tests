...
public class ImportRunnerProvider {
    public static List<ImportRunner> getAllRunners(WorkbenchDeployment workbenchDeployment) {
        List<ImportRunner> runners = new ArrayList<>();
        Stream.of(Users.class.getEnumConstants())
              .forEach(user -> {runners.add(new ImportRunner(workbenchDeployment, user.getName(), user.getPassword()));});
        return runners;
    }
